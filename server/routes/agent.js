...
const AGENT_WORKSPACE_ROOT = path.join(WORKSPACES_ROOT, '.agent');
await fs.mkdir(AGENT_WORKSPACE_ROOT, { recursive: true });
...
async function cloneGitHubRepo(githubUrl, githubToken = null, projectPath) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const cloneDir = validation.resolvedPath;
...
async function cleanupProject(projectPath, sessionId = null) {
  try {
    const realAgentRoot = await fs.realpath(AGENT_WORKSPACE_ROOT);
    const resolvedProjectPath = path.resolve(projectPath);
    if (!isPathWithin(realAgentRoot, resolvedProjectPath) || resolvedProjectPath === realAgentRoot) {
 ...
 *
 *                        Claude models: 'claude-sonnet-4-6' (default), 'claude-opus-4-8',
  ...
  try {
    ...
    if (githubUrl) {
      ...
      if (projectPath) {
        ...
        targetPath = path.join(AGENT_WORKSPACE_ROOT, repoHash);
      }
      ...
      const validation = await validateWorkspacePath(projectPath);
      if (!validation.valid) throw new Error(validation.error);
      finalProjectPath = validation.resolvedPath;
