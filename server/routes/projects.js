...
function createGitCloneEnvironment(githubToken) {
...
// Keep file, upload and terminal operations inside one explicit root. Operators
// can widen this with WORKSPACES_ROOT after reviewing the security implications.
export const WORKSPACES_ROOT = path.resolve(
  process.env.WORKSPACES_ROOT || path.join(os.homedir(), 'CloudCLIWorkspaces'),
);
...
async function getGithubTokenById(tokenId, userId) {
  const credential = credentialsDb.getCredentialById(userId, tokenId, 'github_token');
...
/**
 * Clone repository with progress streaming (SSE framing over fetch stream)
...
router.post('/clone-progress', async (req, res) => {
  ...
  try {
    ...
    let githubToken = null;
    if (githubTokenId) {
      const tokenId = Number.parseInt(githubTokenId, 10);
      if (!Number.isInteger(tokenId)) {
        await fs.rm(absolutePath, { recursive: true, force: true });
        sendEvent('error', { message: 'Invalid GitHub token ID' });
        res.end();
        return;
      }
      const token = await getGithubTokenById(tokenId, req.user.id);
      if (!token) {
        await fs.rm(absolutePath, { recursive: true, force: true });
...
// EventSource GET mode is deprecated because it forces credentials into URL query params.
router.get('/clone-progress', (req, res) => {
  res.status(405).json({
    error: 'Use POST /api/projects/clone-progress with Authorization header'
  });
});
