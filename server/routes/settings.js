...
import { apiKeysDb, credentialsDb, preferencesDb } from '../database/db.js';
...
// ===============================
// User Preferences（跨设备持久化键值对）
// ===============================

// 读取全部偏好设置
router.get('/preferences', (req, res) => {
  try {
    const prefs = preferencesDb.getAll(req.user.id);
...
router.put('/preferences/:key', (req, res) => {
  try {
    ...
    // 只允许字符串类型 value（复杂对象请先 JSON.stringify）
    const strValue = typeof value === 'string' ? value : JSON.stringify(value);
    preferencesDb.set(req.user.id, key, strValue);
