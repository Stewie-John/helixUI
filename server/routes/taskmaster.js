...
async function resolveTaskProjectPath(projectName) {
...
router.get('/detect/:projectName', async (req, res) => {
    try {
        ...
        let projectPath;
        try {
            projectPath = await resolveTaskProjectPath(projectName);
        } catch (error) {
        ...
        const detectionPromises = projects.map(async (project) => {
            try {
                ...
                if (project.fullPath) {
                    ...
                    try {
                        projectPath = await resolveTaskProjectPath(project.name);
                    } catch (error) {
