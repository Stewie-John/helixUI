...
function spawnAsync(command, args, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code) => {
      ...
      const error = new Error(stderr.trim() || `Command failed: ${command} ${args.join(' ')}`);
      error.code = code;
      error.stdout = stdout;
      error.stderr = stderr;
      reject(error);
...
async function getActualProjectPath(projectName) {
  ...
  if (!validation.valid) {
    throw new Error(validation.error || 'Project is outside the configured workspace root');
  }
  return validation.resolvedPath;
}

async function resolveGitFile(projectPath, file, options = {}) {
  if (typeof file !== 'string' || !file.trim()) throw new Error('File path is required');
  const resolved = await resolvePathWithinRoot(projectPath, file, options);
  return { absolutePath: resolved.path, relativePath: path.relative(projectPath, resolved.path) || '.' };
}

async function validateBranchName(branch, cwd) {
  if (typeof branch !== 'string' || !branch.trim() || branch.length > 255) {
    throw new Error('Invalid branch name');
  }
  await spawnAsync('git', ['check-ref-format', '--branch', branch], { cwd });
  return branch;
}
...
router.get('/diff', async (req, res) => {
  ...
  try {
    ...
    const { absolutePath: safeFilePath, relativePath: safeFile } = await resolveGitFile(projectPath, file);
    const { stdout: statusOutput } = await spawnAsync('git', ['status', '--porcelain', '--', safeFile], { cwd: projectPath });
    ...
    if (isUntracked) {
      ...
      const { stdout: fileContent } = await spawnAsync('git', ['show', `HEAD:${safeFile}`], { cwd: projectPath });
      const lines = fileContent.split('\n');
      diff = `--- a/${file}\n+++ /dev/null\n@@ -1,${lines.length} +0,0 @@\n` +
             lines.map(line => `-${line}`).join('\n');
    } else {
      ...
      const { stdout: unstagedDiff } = await spawnAsync('git', ['diff', '--', safeFile], { cwd: projectPath });
      ...
      if (unstagedDiff) {
        ...
        const { stdout: stagedDiff } = await spawnAsync('git', ['diff', '--cached', '--', safeFile], { cwd: projectPath });
...
router.get('/file-with-diff', async (req, res) => {
  ...
  try {
    ...
    if (isDeleted) {
      ...
      const { stdout: headContent } = await spawnAsync('git', ['show', `HEAD:${safeFile}`], { cwd: projectPath });
      ...
      currentContent = await fs.readFile(safeFilePath, 'utf-8');
...
router.post('/initial-commit', async (req, res) => {
  ...
  try {
    ...
    // Add all files
    await spawnAsync('git', ['add', '--all'], { cwd: projectPath });
...
router.post('/commit', async (req, res) => {
  ...
  try {
    ...
    for (const file of files) {
      const { relativePath } = await resolveGitFile(projectPath, file, { allowMissing: true });
      await spawnAsync('git', ['add', '--', relativePath], { cwd: projectPath });
    }
    
    // Commit with message
    const { stdout } = await spawnAsync('git', ['commit', '-m', String(message).slice(0, 10000)], { cwd: projectPath });
...
router.post('/checkout', async (req, res) => {
  ...
  try {
    const projectPath = await getActualProjectPath(project);
    await validateGitRepository(projectPath);
    await validateBranchName(branch, projectPath);
    // Checkout the branch
    const { stdout } = await spawnAsync('git', ['switch', branch], { cwd: projectPath });
...
router.post('/create-branch', async (req, res) => {
  ...
  try {
    ...
    const { stdout } = await spawnAsync('git', ['switch', '-c', branch], { cwd: projectPath });
...
router.get('/commit-diff', async (req, res) => {
  ...
  try {
    ...
    const { stdout } = await spawnAsync('git', ['show', safeCommit], { cwd: projectPath });
...
router.post('/generate-commit-message', async (req, res) => {
  ...
  try {
    ...
    for (const file of files) {
      try {
        const { relativePath } = await resolveGitFile(projectPath, file, { allowMissing: true });
        const { stdout } = await spawnAsync('git', ['diff', 'HEAD', '--', relativePath], { cwd: projectPath });
        if (stdout) {
          diffContext += `\n--- ${file} ---\n${stdout}`;
        }
      } catch (error) {
    ...
    if (!diffContext.trim()) {
      ...
      for (const file of files) {
        try {
          const { absolutePath: filePath } = await resolveGitFile(projectPath, file);
...
router.post('/discard', async (req, res) => {
  ...
  try {
    ...
    const { absolutePath: filePath, relativePath: safeFile } = await resolveGitFile(projectPath, file);
    const { stdout: statusOutput } = await spawnAsync('git', ['status', '--porcelain', '--', safeFile], { cwd: projectPath });
    ...
    if (status === '??') {
      ...
      await spawnAsync('git', ['restore', '--', safeFile], { cwd: projectPath });
    } else if (status.includes('A')) {
      // Added file - unstage it
      await spawnAsync('git', ['reset', 'HEAD', '--', safeFile], { cwd: projectPath });
    }
