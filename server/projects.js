...
const PROJECTS_CACHE_TTL = 3000; // 3 秒

async function getProjects(progressCallback = null) {
  // 有缓存且未过期时直接返回（progressCallback 场景跳过缓存）
  if (!progressCallback && _projectsCache && (Date.now() - _projectsCacheTime < PROJECTS_CACHE_TTL)) {
          ...
          try {
            ...
            if (!foundUserMsg && entry.message?.role === 'user' && entry.message?.content) {
              ...
              if (typeof textContent === 'string' && textContent.length > 0 &&
                ...
                session.summary = textContent.length > 80 ? textContent.substring(0, 80) + '...' : textContent;
...
// 快速解析 Codex session 文件头部（只读前 20 行获取 metadata）
  ...
  try {
    ...
    for (const line of lines) {
      try {
        ...
        if (!firstUserMessage && entry.type === 'event_msg' && entry.payload?.type === 'user_message' && entry.payload.message) {
...
async function getCodexSessions(projectPath, options = {}) {
  const { limit = 5, indexRef = null, prebuiltIndex = null } = options;
  try {
