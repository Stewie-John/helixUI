...
async function spawnGemini(command, options = {}, ws) {
    ...
    if (images && images.length > 0) {
        try {
            ...
            if (tempImagePaths.length > 0) {
                ...
                const modifiedCommand = baseCommand + imageNote;
                effectiveCommand = modifiedCommand;
                ...
                if (promptIndex !== -1 && args[promptIndex + 1] === command) {
                    ...
                    args.push('--prompt', modifiedCommand);
                }
            }
        } catch (error) {
    ...
    const usageEstimate = {
        inputTokens: estimateTokenCount(effectiveCommand) + tempImagePaths.length * 256,
        outputTokens: 0
    };
    ...
    return new Promise((resolve, reject) => {
        ...
        geminiProcess.startedAt = startedAt;
        ...
        if (ws) {
            responseHandler = new GeminiResponseHandler(ws, {
                onContentFragment: (content) => {
                    assistantTextEstimate += content;
                    usageEstimate.outputTokens = Math.max(
                        usageEstimate.outputTokens,
                        estimateTokenCount(assistantTextEstimate)
                    );
                ...
                getTokenUsage: tokenUsagePayload
            });
        }
...
function getActiveGeminiSessions() {
    return Array.from(activeGeminiProcesses.entries()).map(([id, process]) => ({
        id,
        status: 'running',
...
function getGeminiSessionInfo(sessionId) {
