...
export default defineConfig(({ command, mode }) => {
  ...
  return {
    ...
    build: {
      ...
      // Shell is lazy-loaded from MainContent. Keep its xterm dependency out of
      // the initial Chat page preload; it is fetched only after opening Shell.
      modulePreload: {
        resolveDependencies: (_filename, dependencies) =>
          dependencies.filter((dependency) => !dependency.includes('vendor-xterm-')),
      },
      rollupOptions: {
        output: {
          manualChunks: {
