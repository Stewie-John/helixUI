...
## [1.23.0](https://github.com/siteboon/claudecodeui/compare/v1.22.0...v1.23.0) (2026-05-19)

### New Features

* **providers:** add custom provider support (DeepSeek, Qwen, Mistral, etc.) via ANTHROPIC_BASE_URL injection — configure name, model, base URL and API key in Settings → API
* **providers:** custom providers appear as selectable options alongside Claude/Cursor/Codex/Gemini in new session screen
* **image-api:** make image serving root configurable via WORKSPACES_ROOT environment variable instead of hardcoded path

### Bug Fixes

* **shell:** fix reconnected terminal showing blank content after page refresh — added PTY zombie detection and buffer replay on reconnect
* **ui:** fix tool output blocks auto-collapsing when user manually expands them during or after streaming — filter React-driven toggle events using isTrusted
* **ui:** fix key instability causing collapsible sections to remount on every streaming update — reuse stable intrinsic keys (toolId/messageId) across object recreations
* **ui:** fix chat stuck in idle state after long tool executions — extend stale safety timer to 120s and reset sentinel on timeout to allow UI recovery

### Maintenance

* add `data/` and `.claude/settings.local.json` to .gitignore
