    ...
    nextTerminal.attachCustomKeyEventHandler((event) => {
      ...
      if (
        ...
        const sendText = (text: string) => {
          if (text) sendSocketMessage(wsRef.current, { type: 'input', data: text });
        };

        // Fallback: temporary textarea trick (no clipboard-read permission needed)
        const pasteViaTextarea = () => {
          ...
          const text = ta.value;
          ta.remove();
          if (ok && text) sendText(text);
        };

        if (typeof navigator !== 'undefined' && navigator.clipboard?.readText) {
          navigator.clipboard.readText().then(sendText).catch(pasteViaTextarea);
        } else {
          pasteViaTextarea();
        }
