...
export function useShellRuntime({
  ...
  const isUserScrollingShellRef = useRef(false);
  ...
  useEffect(() => {
    ...
    const providerKey = selectedSession?.__provider || '';
    const contextKey = `${projectKey}::${currentSessionId || ''}::${providerKey}::${initialCommand || ''}::${isPlainShell ? 'plain' : 'agent'}`;

    if (!lastShellContextKeyRef.current) {
