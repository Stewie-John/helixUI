...
export default function AppearanceSettingsTab({
  ...
  const { avatarUrl, uploadAvatar, removeAvatar } = useUserAvatar();
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-semibold text-foreground">{t('appearanceSettings.avatar.title')}</h3>
        <div className="bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
          <div className="flex items-center gap-4">
            {/* 头像预览 */}
            <div className="w-16 h-16 rounded-full overflow-hidden bg-blue-600 flex items-center justify-center text-white text-2xl flex-shrink-0">
              {avatarUrl
            ...
            <div className="flex-1">
              <div className="font-medium text-foreground mb-1">{t('appearanceSettings.avatar.label')}</div>
              <div className="text-sm text-muted-foreground mb-3">
                {t('appearanceSettings.avatar.description')}
              </div>
              <div className="flex gap-2">
                ...
                >
                  {avatarUrl ? t('appearanceSettings.avatar.change') : t('appearanceSettings.avatar.upload')}
                </button>
                {avatarUrl && (
                  <button
                    onClick={() => { void removeAvatar().catch(err => console.error('Avatar remove failed:', err)); }}
                    className="px-3 py-1.5 text-sm bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 rounded-md transition-colors"
                  >
                    {t('appearanceSettings.avatar.reset')}
                  </button>
                )}
              </div>
              <input
