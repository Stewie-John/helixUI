...
function Settings({ isOpen, onClose, projects = [], initialTab = 'agents' }: SettingsProps) {
  ...
  const auth = useAuth() as unknown as { user: { id?: number } | null };
    ...
    <div className="modal-backdrop fixed inset-0 flex items-center justify-center z-[9999] md:p-4 bg-background/95">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          <SettingsMainTabs activeTab={activeTab} onChange={setActiveTab} isAdmin={isAdmin} />
          ...
          <div className="p-4 md:p-6 space-y-6 md:space-y-8 pb-safe-area-inset-bottom">
            ...
            {activeTab === 'users' && (
              <div className="space-y-6 md:space-y-8">
                <UsersSettingsTab isAdmin={isAdmin} />
              </div>
            )}
          </div>
        </div>
