...
const isHiddenRuntimeUserText = (text: string) => {
...
const getVisibleRuntimeUserText = (text: string) => {
...
/**
 * 将 tool_result 的 content 字段（可能是字符串或块数组）安全地转换为字符串。
 * - 纯字符串：直接返回
 * - 数组（含 text/image/tool_reference 块）：提取 text 部分；忽略 base64 图片块
 */
  ...
  for (const block of content) {
    if (block && typeof block === 'object') {
      ...
      if (b.type === 'text' && typeof b.text === 'string') {
        ...
        // 跳过 base64 图片块，避免巨型字符串进入渲染管道
      } else if (b.type === 'tool_reference') {
        // 工具引用：仅显示工具名
        if (typeof b.tool_name === 'string') {
          textParts.push(`[tool: ${b.tool_name}]`);
        }
      } else {
        // 其他未知块：尝试 JSON 化
        try { textParts.push(JSON.stringify(b)); } catch { /* ignore */ }
      }
    }
  }
...
export const convertSessionMessages = (rawMessages: any[]): ChatMessage[] => {
  ...
  rawMessages.forEach((message) => {
    // 优先使用 isCompactSummary 标志：将压缩消息折叠显示，不完全跳过
    if (message.isCompactSummary === true && message.message?.role === 'user' && message.message?.content) {
      const rawContent = typeof message.message.content === 'string'
        ? message.message.content
        : Array.isArray(message.message.content)
          ? message.message.content.map((p: any) => (typeof p === 'string' ? p : p?.text || '')).join('\n')
    ...
    if (message.message?.role === 'user' && message.message?.content) {
      ...
      content = getVisibleRuntimeUserText(content);

      const shouldSkip =
        !content ||
        isHiddenRuntimeUserText(content) ||
        ...
        content.startsWith('The previous conversation was automatically compacted') ||
        content.startsWith('[Request interrupted');

      if (!shouldSkip) {
        // Parse <task-notification> blocks into compact system messages
        const taskNotifRegex = /<task-notification>[\s\S]*?<status>([^<]*)<\/status>[\s\S]*?<summary>([^<]*)<\/summary>[\s\S]*?<\/task-notification>/g;
      ...
      if (Array.isArray(message.message.content)) {
        message.message.content.forEach((part: any) => {
          if (part.type === 'text') {
            ...
            // 过滤 SDK 产生的无意义占位文本（Claude 仅调用工具未输出文字时自动生成）
          ...
          if (part.type === 'tool_use') {
            ...
            converted.push({
              id: message.id ? `${message.id}:tool:${part.id || 'unknown'}` : undefined,
              type: 'assistant',
              content: '',
              timestamp: message.timestamp || new Date().toISOString(),
              isToolUse: true,
