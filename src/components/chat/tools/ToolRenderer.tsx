...
function getToolCategory(toolName: string): string {
  if (['Edit', 'Write', 'ApplyPatch', 'Editing', 'FileChanges'].includes(toolName)) return 'edit';
  if (['Grep', 'Glob', 'Web search', 'WebSearch'].includes(toolName)) return 'search';
  const normalized = String(toolName || '').toLowerCase().replace(/[^a-z0-9]/g, '');
  if (normalized === 'bash' || normalized === 'exec' || normalized === 'wait' || normalized.endsWith('execcommand')) return 'bash';
