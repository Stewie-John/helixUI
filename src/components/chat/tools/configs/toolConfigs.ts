...
const formatCodexToolStatus = (status: unknown) => {
  if (!status) return undefined;
  const text = String(status).replace(/_/g, ' ');
  return text === 'in progress' ? 'running' : text;
};

function decodeCommandLiteral(value: string, quote: string) {
  if (quote === '"') {
    try {
      return JSON.parse(`"${value}"`);
    } catch {
      // Fall through to the lightweight decoder for malformed historical records.
    }
  }
...
function extractCommandFromExecSource(source: string) {
  // Codex session history stores custom-tool input as the orchestrator source,
  // e.g. tools.exec_command({cmd:"rg -n ..."}). Render the command, not that source.
  const match = source.match(/(?:["']?cmd["']?|command)\s*:\s*(["'`])((?:\\.|(?!\1)[\s\S])*)\1/);
  if (!match) return '';
  return decodeCommandLiteral(match[2], match[1]).trim();
}

function getExecCommand(input: any) {
  if (typeof input === 'string') {
    try {
      return getExecCommand(JSON.parse(input));
    } catch {
      return extractCommandFromExecSource(input) || input.trim();
    }
  }

  if (!input || typeof input !== 'object') return '';
  const command = input.cmd || input.command || input.commandLine || input.script || input.code;
  if (typeof command !== 'string') return '';
  return extractCommandFromExecSource(command) || command.trim();
}

function getWaitSummary(input: any) {
  if (typeof input === 'string') {
    try {
      return getWaitSummary(JSON.parse(input));
    } catch {
      return input.trim() ? 'waiting for task' : 'waiting';
    }
  }

  if (!input || typeof input !== 'object') return 'waiting';
  const target = input.cell_id ?? input.session_id ?? input.process_id ?? input.id;
  return target ? `task ${target}` : 'waiting';
}

const formatCodexFileChange = (change: any) => {
  if (!change || typeof change !== 'object') return String(change || '');
  const kind = change.kind || change.type || 'change';
  const filePath = change.path || change.file_path || change.file || '';
  return filePath ? `${kind}: ${filePath}` : String(kind);
};

const getCodexChanges = (input: any) => {
...
export const TOOL_CONFIGS: Record<string, ToolDisplayConfig> = {
  ...
  'Exec command': {
    input: {
      ...
      label: 'Exec command',
      getValue: (input) => input.command || '',
      getSecondary: (input) => formatCodexToolStatus(input.status),
      action: 'copy',
      style: 'terminal',
      wrapText: true,
      colorScheme: {
  ...
  // Codex CLI custom tool names preserved in historical session transcripts.
  exec: {
    input: {
      type: 'one-line',
      icon: 'terminal',
      label: 'Ran',
      getValue: getExecCommand,
      action: 'copy',
      style: 'terminal',
      wrapText: true,
    },
  ...
  'MCP tool': {
    input: {
      type: 'collapsible',
      title: (input) => {
        const target = `${input.server || 'MCP'}:${input.tool || 'tool'}`;
        const status = formatCodexToolStatus(input.status);
  ...
  Editing: {
    input: {
      type: 'collapsible',
      title: (input) => {
        const changes = getCodexChanges(input);
        const status = formatCodexToolStatus(input?.status);
        const summary = changes.length === 1 ? '1 file change' : `${changes.length} file changes`;
      ...
      getContentProps: (input) => {
        const changes = getCodexChanges(input);
        return {
          content: changes.map(formatCodexFileChange).join('\n') || 'Editing files',
          format: 'code'
        };
      }
    },
  ...
  FileChanges: {
    input: {
      ...
      title: (input) => {
        const changes = getCodexChanges(input);
        return changes.length === 1 ? '1 file change' : `${changes.length} file changes`;
      },
      defaultOpen: false,
      contentType: 'text',
      getContentProps: (input) => ({
        content: getCodexChanges(input).map(formatCodexFileChange).join('\n') || String(input || ''),
        format: 'code'
      })
    },
    result: {
  ...
  'Web search': {
    input: {
      type: 'one-line',
      label: 'Web search',
      getValue: (input) => input.query || 'searching',
      getSecondary: (input) => formatCodexToolStatus(input.status),
      action: 'none',
      colorScheme: {
  ...
  TodoWrite: {
    input: {
      ...
      title: (input) => {
        const status = formatCodexToolStatus(input?.status);
        return status ? `Updating todo list · ${status}` : 'Updating todo list';
      },
      defaultOpen: false,
...
export function getToolConfig(toolName: string): ToolDisplayConfig {
  ...
  if (normalized === 'exec' || normalized.endsWith('execcommand')) {
    return TOOL_CONFIGS['Exec command'];
  }
