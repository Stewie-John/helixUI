...
export function useFileMentions({ selectedProject, input, setInput, textareaRef }: UseFileMentionsOptions) {
  ...
  const mentionRequested = useMemo(() => {
    const beforeCursor = input.slice(0, cursorPosition);
    const lastAt = beforeCursor.lastIndexOf('@');
    return lastAt >= 0 && !beforeCursor.slice(lastAt + 1).includes(' ');
  ...
  useEffect(() => {
    ...
    const fetchProjectFiles = async () => {
      ...
      try {
        const response = await api.getMentionFiles(projectName, { signal: abortController.signal });
        if (!response.ok) {
          return;
        }
  ...
  }, [mentionRequested, selectedProject?.name]);
