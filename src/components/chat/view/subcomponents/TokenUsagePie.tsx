...
export default function TokenUsagePie({ used, total }: TokenUsagePieProps) {
  ...
  const bgSegs = Array.from({ length: 20 }, (_, i) => {
    const p = i * 5;
    return { d: arcPath(p, p + 5), color: rgb(getGaugeColor(p + 2.5), 0.18) };
  });

  // 前景弧（已使用量，每2%一段，全亮度渐变色）
  const fgSegs: { d: string; color: string }[] = [];
  for (let p = 0; p < percentage; p += 2) {
    const p2 = Math.min(p + 2, percentage);
    fgSegs.push({ d: arcPath(p, p2), color: rgb(getGaugeColor((p + p2) / 2)) });
  }
  ...
  return (
    <div className="flex items-center gap-1.5 text-xs">
      ...
      <svg width="48" height="27" viewBox="0 0 48 27">
        {/* 背景弧：暗淡的全量程渐变 */}
        {bgSegs.map((s, i) => (
          <path key={`bg${i}`} d={s.d} fill="none" stroke={s.color} strokeWidth="3" strokeLinecap="butt" />
        ))}
        {/* 前景弧：已使用量，亮色渐变（左绿→右红） */}
        {fgSegs.map((s, i) => (
          <path key={`fg${i}`} d={s.d} fill="none" stroke={s.color} strokeWidth="3" strokeLinecap="butt" />
        ))}
        {/* 刻度线：长短交替（20的倍数为长线） */}
        {ticks.map((t, i) => (
        ...
        <line x1={cx} y1={cy} x2={nx} y2={ny}
          stroke={rgb(needleColor)} strokeWidth="1.8" strokeLinecap="round" />
        {/* 中心圆点 */}
        <circle cx={cx} cy={cy} r="2" fill={rgb(curColor)} />
      </svg>
      <span style={{ color: rgb(curColor) }} title={`${used.toLocaleString()} / ${total.toLocaleString()} tokens`}>
