...
const CodeBlock = ({ node: _node, inline, className, children, ...props }: CodeBlockProps) => {
    ...
    return (
      <div
        className={`flex items-center gap-2 my-2 px-3 py-2 font-mono text-sm cursor-pointer transition-colors ${
          isTech
            ? 'rounded-none border-l-2 border-cyan-400/60 bg-transparent text-green-400 hover:border-cyan-400'
      ...
      >
        ...
        <span className="flex-1 truncate text-gray-400">{previewText}</span>
  ...
  // 展开状态：完整代码块
  // tech 主题用原生 <pre>（绕过 SyntaxHighlighter 的内联 style 黑背景）
  // 其他主题用 SyntaxHighlighter + oneDark
  const copyBtn = (
    <button
      type="button"
      onClick={() =>
  ...
  if (isTech) {
    // tech 主题：原生 <pre>，透明背景 + 绿色发光，不使用 SyntaxHighlighter
    return (
      <div className="relative group my-2" style={{ borderLeft: '2px solid rgba(0,220,180,0.40)' }}>
        {language && language !== 'text' && (
          <div className="absolute top-2 left-3 z-10 text-xs font-medium uppercase"
            style={{ color: 'rgba(0,220,180,0.7)', letterSpacing: '0.1em' }}>
            {language}
          </div>
        )}
        {copyBtn}
        <pre style={{
          ...
          color: '#00e87a',
          textShadow: '0 0 6px rgba(0,232,120,0.50)',
...
export function Markdown({ children, className, messageTimestamp = 0 as string | number | Date }: MarkdownProps) {
  ...
  const content = normalizeInlineCodeFences(injectImagePaths(raw));
