  ...
  const handleCreate = async () => {
    ...
    try {
      if (workspaceType === 'new' && githubUrl) {
        ...
        const response = await api.post('/projects/clone-progress', payload);
        if (!response.ok) {
          let errorMessage = `Clone request failed (${response.status})`;
          try {
            const data = await response.json();
        ...
        if (!response.body) {
          throw new Error('Clone response stream unavailable');
        }
        ...
        while (true) {
          ...
          while ((boundaryIndex = buffer.indexOf('\n\n')) !== -1) {
            ...
            try {
              ...
              console.error('Error parsing clone stream event:', e);
            }
          }
        }

        if (!completed) {
          throw new Error('Connection closed before clone completed');
        }
        return;
      }
