...
export default function SidebarFolderNode({
  ...
  const sessionsInThisFolder = sessions.filter(
    (s) => folderOfSession(s.id, s.__provider) === node.id,
  );
  ...
  return (
    <div className="space-y-0.5">
      <div
        ...
        onDrop={handleDrop}
        className={cn(
          'group flex items-center gap-1 px-1.5 py-1 rounded-md hover:bg-accent/40 cursor-pointer transition-colors',
      ...
      >
        ...
        {renaming ? (
          ...
          <>
            ...
            <span className="text-[10px] text-muted-foreground opacity-70 group-hover:opacity-0 transition-opacity">
              {sessionsInThisFolder.length + node.children.length || ''}
            </span>
